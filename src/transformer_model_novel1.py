...
class AttentionEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        self.prot_ff = FeedForwardLayer(d_model, dim_feedforward, dropout, activation)
        self.comp_ff = FeedForwardLayer(d_model, dim_feedforward, dropout, activation)
    ...
    def forward(self, prot: Tensor, comp: Tensor, prot_mask: Optional[Tensor] = None, comp_mask: Optional[Tensor] = None,
        ...
        x1 = self.prot_norm3(x1 + self.prot_ff(x1))
        y1 = self.comp_norm3(y1 + self.comp_ff(y1))
