...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, d_model, init_bonds, decoder_layer, affinity_output_layer, pairwise_output_layer, num_layers, norm=None):
    ...
    def forward(self, tgt: Tensor, edge: Tensor, atom_adj: Tensor, bond_adj: Tensor, nbs_mask: Tensor,
        ...
        vertex_feature = tgt
        # test case
        # 1. add cross-attention and feed-forward layer to original structure without normalization
        # 2. adjust the order of graph layer and attention layer
        # 3. add normalization based on 1
        # 4. add normalization based on 2
        # 4. adjust the layers of decoder
        for GWM_iter in range(self.num_layers):
            ...
            vertex_feature = self.layers[GWM_iter](vertex_feature, memory, tgt_mask=tgt_mask, memory_mask=memory_mask, 
                                                   tgt_key_padding_mask=tgt_key_padding_mask,
                                                   memory_key_padding_mask=memory_key_padding_mask)
