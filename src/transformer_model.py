...
class Transformer(nn.Module):
    ...
    def __init__(self, init_atoms, init_residues, d_encoder: int = 512, d_decoder: int = 512, d_model: int = 512, 
        ...
        if custom_encoder is not None:
            ...
            encoder_transform_layer = LinearTransformLayer(d_encoder, d_model, dropout, activation, **factory_kwargs)
            ...
            self.encoder = TransformerEncoder(
                encoder_transform_layer, encoder_layer, num_encoder_layers, encoder_norm)

        if custom_decoder is not None:
            self.decoder = custom_decoder
        else:
            decoder_transform_layer = LinearTransformLayer(d_decoder, d_model, dropout, activation, **factory_kwargs)
            ...
            self.decoder = TransformerDecoder(
                decoder_transform_layer, decoder_layer, affinity_output_layer, pairwise_output_layer, num_decoder_layers, decoder_norm)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_transform_layer, encoder_layer, num_layers, norm=None, enable_nested_tensor=False):
...
class PairwiseOutputLayer(nn.Module):
    ...
    def __setstate__(self, state):
        ...
        super(PairwiseOutputLayer, self).__setstate__(state)
...
class AffinityOutputLayer(nn.Module):
    ...
    def __setstate__(self, state):
        if 'activation' not in state:
            state['activation'] = F.relu
        super(AffinityOutputLayer, self).__setstate__(state)
...
class LinearTransformLayer(nn.Module):
    def __init__(self, d_input: int, d_output : int, dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.relu,
                 device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        ...
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_input, d_output, **factory_kwargs)
        self.dropout = nn.Dropout(dropout)
    ...
    def __setstate__(self, state):
        if 'activation' not in state:
            state['activation'] = F.relu
        super(LinearTransformLayer, self).__setstate__(state)
