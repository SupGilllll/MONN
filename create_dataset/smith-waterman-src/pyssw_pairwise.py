...
# iterate query sequence
    querys = read(args.query)
    targets = read(args.target)
    #print 'len querys', len(querys), 'len targets', len(targets) 
    
    for i in range(len(querys)):
        sQId,sQSeq,sQQual = querys[i]
# build query profile
...
# set mask len
        if len(sQSeq) > 30:
            nMaskLen = old_div(len(sQSeq), 2)
        else:
            nMaskLen = 15

# iter target sequence
        sRId,sRSeq,_ = targets[i]
