...
def get_uniprotid_mapping():
	uniprotid_mapping = {}
	with open('./out1.6_uniprot_uniprot_mapping.tab') as f:
		for line in f.readlines()[1:]:
			lines = line.strip().split('\t')
			uniprotid_mapping[lines[0]] = lines[1]
	print('mapping length', len(uniprotid_mapping))
	return uniprotid_mapping
...
def get_fasta_dict():
	...
	# 	for line in f.readlines()[1:]:
	# 		lines = line.strip().split('\t')
	# 		if lines[0] not in uniprot_dict and lines[1] in uniprot_dict:
	# 			uniprot_dict[lines[0]] = uniprot_dict[lines[1]]
...
def get_pdbid_to_uniprotid(seq_dict):
	...
	with open('out1.4_pdb_uniprot_mapping.tab') as f:
		for line in f.readlines()[1:]:
			lines = line.strip().split('\t')
			pdbid_to_uniprotid[lines[0]] = lines[1]
...
uniprotid_mapping = get_uniprotid_mapping()
...
error_step1, error_step2, error_step3, error_step4, error_step5 = 0, 0, 0, 0, 0
for pdbid in pdbid_to_uniprotid:
	...
	if pdbid_to_uniprotid[pdbid] not in uniprotid_mapping:
		error_step5 += 1
		continue
	uniprotid = uniprotid_mapping[pdbid_to_uniprotid[pdbid]]
...
print('fail_step1-4', error_step1, error_step2, error_step3, error_step4, error_step5)
