...
def train_and_eval(train_data, valid_data, test_data, params, batch_size=32, num_epoch=30):
    ...
    for epoch in range(num_epoch):
        ...
        for i in range(math.ceil(len(train_data[0])/batch_size)):
            ...
            vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask = batch_data_process(inputs)
            
            affinity_label = torch.FloatTensor(affinity_label).cuda()
            pairwise_mask = torch.FloatTensor(pairwise_mask).cuda()
            pairwise_label = torch.FloatTensor(pad_label_2d(pairwise_label, vertex, seq_mask)).cuda()
            
            optimizer.zero_grad()
            affinity_pred, pairwise_pred = net(vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, pids)
...
if __name__ == "__main__":
    ...
    assert embed_model in ['t33', 't36']
    embedding_dim = {'t33' : 1280, 't36' : 2560, 't48' : 5120}
    ...
    params = [GNN_depth, inner_CNN_depth, DMA_depth, k_head, kernel_size, hidden_size1, hidden_size2, embed_model, embedding_dim[embed_model]]
